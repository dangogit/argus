...
def test_auto_changes_coalesce_with_existing_converse_request(conn, tmp_path):
    cfg = _cfg_two_projects(tmp_path, authority="auto-changes")
    day = date(2026, 6, 18)
    task = "Coalesce equivalent PM requests before creating work"
    converse_fp = "converse:323e8971-0e4b-45b1-bd12-008792c37b75"
    eid = events.ingest_message(
        conn, cfg, team="dev", source="slack", dedup_key="owner-1", text=task,
    )
    ...
    with conn.cursor() as cur:
        ...
        assert cur.fetchone()[0] == existing_request
