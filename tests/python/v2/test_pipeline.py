...
def test_enqueue_stage_sets_checkpoint_guidance_in_snapshot(conn, cfg):
    ...
    assert "every known recurring failure" in snap["checkpoints"]
    assert "replayed against the change" in snap["checkpoints"]
    assert "rejected with recorded evidence" in snap["checkpoints"]
