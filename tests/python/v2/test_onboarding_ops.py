...
def test_onboard_pm_mode_sets_safe_pr_defaults(tmp_path):
    ...
    assert "Before marking qa-fail" in roles["qa"]["prompt"]
    assert "environment blocker" in roles["qa"]["prompt"]
    assert "auth blocker" in roles["qa"]["prompt"]
    assert "access blocker" in roles["qa"]["prompt"]
    assert "app-code regression" in roles["qa"]["prompt"]
    assert "Do not classify environment, auth, or access blockers as app-code regressions" in roles["qa"]["prompt"]
