...
def test_bundled_project_rules_matches_developer_work():
    ...
    assert "covered items" in block
    assert "disposition for every covered report or item" in block
    assert "verification evidence" in block
    assert "unresolved follow-up condition" in block
    assert "retro-change:4cd623fed543ba165d781063" in block
    assert "retro-change:bb3a1f7887584ccc49e85f87" in block
    assert "retro-change:2c0d2299d85d2858e4aa6f67" in block
    assert "retro-change:2c1f44510a3ec90e2eda8cd6" in block
