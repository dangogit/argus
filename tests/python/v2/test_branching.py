...
def test_stale_senior_failure_ignores_approve_when_latest_run_failed(conn, cfg_project,
                                                                     monkeypatch,
                                                                     tmp_path):
    cfg_project.team("dev").pipeline.max_iters = 0
    cfg_project.team("dev").project.autofix.force_draft_on_fail = False
    rid = _open(conn, cfg_project, text="fix checkout total"); conn.commit()
    ...
    assert later_senior.request_id == rid
    with conn.cursor() as cur:
        cur.execute("UPDATE runs SET status='failed' WHERE job_id=%s", (later_senior.id,))
    conn.commit()
